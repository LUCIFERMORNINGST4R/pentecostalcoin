...
//
#ifndef PENTECOSTALCOIN_BASE58_H
#define PENTECOSTALCOIN_BASE58_H
...
/** base58-encoded Pentecostalcoin addresses.
 * Public-key-hash-addresses have version 0 (or 111 testnet).
 * The data vector contains RIPEMD160(SHA256(pubkey)), where pubkey is the serialized public key.
 * Script-hash-addresses have version 5 (or 196 testnet).
 * The data vector contains RIPEMD160(SHA256(cscript)), where cscript is the serialized redemption script.
 */
class CPentecostalcoinAddress;
class CPentecostalcoinAddressVisitor : public boost::static_visitor<bool>
{
private:
    CPentecostalcoinAddress *addr;
public:
    CPentecostalcoinAddressVisitor(CPentecostalcoinAddress *addrIn) : addr(addrIn) { }
    bool operator()(const CKeyID &id) const;
    bool operator()(const CScriptID &id) const;
    bool operator()(const CNoDestination &no) const;
};

class CPentecostalcoinAddress : public CBase58Data
{
public:
    enum
    {
    ...
    {
        return boost::apply_visitor(CPentecostalcoinAddressVisitor(this), dest);
    }
    ...
    CPentecostalcoinAddress()
    {
    }

    CPentecostalcoinAddress(const CTxDestination &dest)
    {
        Set(dest);
    }

    CPentecostalcoinAddress(const std::string& strAddress)
    {
        SetString(strAddress);
    }

    CPentecostalcoinAddress(const char* pszAddress)
    {
...
bool inline CPentecostalcoinAddressVisitor::operator()(const CKeyID &id) const         { return addr->Set(id); }
bool inline CPentecostalcoinAddressVisitor::operator()(const CScriptID &id) const      { return addr->Set(id); }
bool inline CPentecostalcoinAddressVisitor::operator()(const CNoDestination &id) const { return false; }

/** A base58-encoded secret key */
class CPentecostalcoinSecret : public CBase58Data
{
public:
    ...
    CPentecostalcoinSecret(const CSecret& vchSecret, bool fCompressed)
    {
        SetSecret(vchSecret, fCompressed);
    }

    CPentecostalcoinSecret()
    {
    }
};

#endif // PENTECOSTALCOIN_BASE58_H
