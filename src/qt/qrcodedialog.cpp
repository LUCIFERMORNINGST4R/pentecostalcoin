...
{
    QString ret = QString("pentecostalcoin:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non PCC unit input in lnReqAmount, we generate the URI with PCC as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(PentecostalcoinUnits::format(PentecostalcoinUnits::PCC, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
